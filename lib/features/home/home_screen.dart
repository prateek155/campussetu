...
class _HomeScreenState extends ConsumerState<HomeScreen> {
    ...
    return Scaffold(
      ...
      floatingActionButton: Padding(
        ...
        child: FloatingActionButton(
          onPressed: () => HelpChatWidget.show(context),
                      ...
                      child: ListView(
                        ...
                        children: [
                          ...
                          _buildCarouselCard(
                            ...
                            title: 'Flatmates',
                            subtitle: 'Find rooms',
                            onTap: () => ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Flatmates feature unlocking soon!'))),
                          ),
                        ],
                      ),
                    ),
  ...
  Widget _buildCarouselCard(BuildContext context, {required IconData icon, required Color iconColor, required String title, required String subtitle, String? badge, required VoidCallback onTap}) {
...
class _CommentsSheetState extends State<_CommentsSheet> {
  ...
  String _serverError(Object e) {
    try {
      ...
      final d = resp?.data;
      if (d is Map && d['error'] != null) return code != null ? '[$code] ${d['error']}' : d['error'].toString();
      if (code != null) return 'Server error [$code]. Retry karo.';
    } catch (_) {}
    final s = e.toString();
    if (s.contains('SocketException') || s.contains('connection')) return 'Network issue. Internet check karo.';
    if (s.contains('Timeout')) return 'Server slow hai. Retry karo.';
  ...
  Widget build(BuildContext context) {
    return Container(
      margin: EdgeInsets.fromLTRB(16, 16, 16, 16 + MediaQuery.of(context).padding.bottom + 76),
