...
Color get _invitationSurface => AppColors.isDark ? const Color(0xFF1B1F2E) : Colors.white;
Color get _invitationBorder => AppColors.isDark ? const Color(0xFF30364A) : const Color(0xFFE5E9EF);
  ...
  Widget build(BuildContext context) {
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        children: [
          Container(
            color: _invitationSurface,
            padding: const EdgeInsets.fromLTRB(16, 12, 16, 12),
            child: Container(
              padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 8),
              decoration: BoxDecoration(color: AppColors.cyanDeep, borderRadius: BorderRadius.circular(20)),
          ...
          if (_pending.isEmpty) ...[
            Container(
              ...
              child: Column(children: [
                Icon(Icons.mark_email_read_outlined, size: 64, color: AppColors.inkSoft),
            ...
            Container(color: _invitationSurface, child: Column(children: _pending.map((item) {
              ...
              return Container(
                ...
                decoration: BoxDecoration(color: _invitationSurface, borderRadius: BorderRadius.circular(17), border: Border.all(color: _invitationBorder)),
                child: Row(children: [
...
class _SuggestionsListState extends State<_SuggestionsList> {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: _users.map((e) {
        final m = Map<String, dynamic>.from(e as Map);
        final name = (m['name'] ?? '').toString();
        return Container(
          margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 6),
          padding: const EdgeInsets.fromLTRB(14, 10, 14, 10),
          decoration: BoxDecoration(color: _invitationSurface, borderRadius: BorderRadius.circular(16), border: Border.all(color: _invitationBorder)),
          child: Row(children: [
            ...
            OutlinedButton(onPressed: () async { final messenger = ScaffoldMessenger.of(context); try { await ApiService().sendConnectionRequest(m['id'].toString()); if (!mounted) return; messenger.showSnackBar(const SnackBar(content: Text('Request sent'))); } catch (e) { if (!mounted) return; messenger.showSnackBar(SnackBar(content: Text('Failed: $e'))); } }, style: OutlinedButton.styleFrom(side: BorderSide(color: AppColors.cyanDeep), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20))), child: Text('Connect', style: TextStyle(color: AppColors.cyanDeep, fontWeight: FontWeight.w700, fontSize: 12))),
          ]),
        );
      }).toList());
  }
        ...
        children: [
          ...
          if (_sent.isEmpty)
            ...
            ..._sent.map((e) {
              ...
              final sentLabel = createdAt == null ? 'Pending' : 'Sent ${DateFormat('d MMM').format(createdAt.toLocal())}';
              return Container(
                margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 6),
                padding: const EdgeInsets.fromLTRB(14, 12, 14, 12),
                decoration: BoxDecoration(color: _invitationSurface, borderRadius: BorderRadius.circular(16), border: Border.all(color: _invitationBorder)),
                child: Row(children: [
                  ...
                  const SizedBox(width: 12),
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(name, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w700)), Text((other['college'] ?? '').toString(), style: AppTypography.interCaption(), maxLines: 1, overflow: TextOverflow.ellipsis), Text(sentLabel, style: AppTypography.interCaption())])),
                  TextButton(onPressed: () async { final messenger = ScaffoldMessenger.of(context); try { await ApiService().removeConnection(id); if (!mounted) return; messenger.showSnackBar(const SnackBar(content: Text('Withdrawn'), backgroundColor: AppColors.success)); _load(); } catch (e) { if (!mounted) return; messenger.showSnackBar(SnackBar(content: Text('Failed: $e'), backgroundColor: AppColors.error)); } }, child: Text('Withdraw', style: TextStyle(color: AppColors.ink, fontWeight: FontWeight.w700))),
                ]),
              );
            }),
        ],
